...
Dans le domaine du pentest (tests de pénétration) et des CTF (Capture The Flag), la phase de reconnaissance est une étape cruciale. C'est là que Nmap entre en jeu. Nmap, abréviation de Network Mapper, est un outil polyvalent et puissant utilisé par les professionnels de la sécurité informatique pour explorer, analyser et sécuriser les réseaux.
...
Dans ce guide, nous explorerons rapidement les différentes options d'analyse qu'offre Nmap et leurs rôles
...
Nmap fonctionne en envoyant des paquets réseau sur le réseau cible et en analysant les réponses pour déterminer quels ports sont ouverts, quels services sont en cours d'exécution sur ces ports, et quelles sont les adresses IP des hôtes actifs sur le réseau. Il utilise différentes techniques de scan (que nous verront dans la section suivante) pour accomplir ces tâches, chacune avec ses propres avantages et inconvénients en termes de discrétion, vitesse et détection.
...
- Les ``scans Null`` (-sN), ``Fin`` (-sF) et ``Xmas`` (-sX) envoient des paquets avec des drapeaux TCP spécifiques pour pénétrer certains pare-feu et extraire des informations.

- Le scan Maimon (-sM) envoie des drapeaux FIN et ACK, principalement pour BSD, mais actuellement, il rapportera tous les ports comme fermés.
...
- L'option -b <server> est utilisée pour scanner un hôte depuis un autre via FTP, mais est de peu d'utilité pratique dans la plupart des cas.


**Paramètres pour les Adresses IP à Scanner**

- ``<ip>,<net/mask>`` : Permet de spécifier des adresses IP individuelles ou des plages d'adresses avec leur masque.
- `-iL <ips_file>`: Charge une liste d'adresses IP à partir d'un fichier spécifié.
- `-iR <number>`: Génère un nombre spécifié d'adresses IP aléatoires à scanner. On peut exclure des adresses avec ``--exclude <Ips>`` ou ``--excludefile <file>``.
...
- ``-sV`` : L'option de scan de version (-sV) permet d'effectuer une analyse des versions des services qui tournent sur les ports ouverts. L'intensité de cette analyse peut être réglée de 0 à 9, avec 7 comme intensité par défaut.

- ``--version-intensity <numero>`` : Cette option permet de régler l'intensité de l'analyse de version, ce qui peut accélérer le scan UDP en limitant le temps passé sur chaque port.

- ``-O`` : Le scan -O effectue une détection d'OS, c'est-à-dire qu'il tente de déterminer le système d'exploitation des hôtes scannés en analysant différentes caractéristiques des paquets réseau.

- ``--osscan-limit`` : Cette option permet de limiter la détection d'OS à des hôtes qui ont au moins un port ouvert et un port fermé. Cela peut réduire le temps de scan en éliminant les hôtes moins intéressants.

- ``--osscan-guess`` : Lorsque la détection d'OS est incertaine, cette option améliore les estimations en analysant plus en profondeur les caractéristiques des réponses réseau.


#### Découverte d'Équipement

Lorsque Nmap lance sa phase de découverte, il utilise par défaut une combinaison de techniques pour identifier les équipements actifs sur le réseau, comprenant : -PA80 -PS443 -PE -PP. Ces options sont des spécifications de types de sondes utilisées par défaut par Nmap lors de la phase de découverte des équipements sur le réseau. Voici ce que chacune de ces options signifie :
...
Ainsi, en utilisant une combinaison de ces sondes, Nmap peut identifier les équipements actifs sur le réseau et établir une base de données sur laquelle il pourra ensuite effectuer des scans plus approfondis des ports et des services
    ...
    -PE, -PP, -PM : Ces options lancent des pings ICMP (écho, timestamp et address mask) pour vérifier si la cible est active.

    -PY<ports> : Envoie des sondes SCTP INIT, généralement à 80 par défaut, pour détecter l'état ouvert/fermé/inactif des ports.
...
- **-p «ports»**: Cette option spécifie les ports à scanner. Vous pouvez spécifier une plage de ports, une liste de ports séparés par des virgules ou utiliser `-p-` ou `-p all` pour scanner tous les 65335 ports.
- ``**-F**``: Cette option effectue un scan rapide des 100 ports les plus couramment utilisés.
- ``**--top-ports «nombre»**``: Cette option vous permet de spécifier le nombre de ports principaux à analyser, compris entre 1 et 65335.
- ``**-r**``: Cette option analyse les ports dans un ordre aléatoire afin d'éviter une détection facile par les systèmes de sécurité.

### -sV: Scan de Version

- ``**-sV**``: Cette option effectue un scan de version des services s'exécutant sur les ports découverts. L'intensité peut être réglée de 0 à 9 (7 par défaut).
- ``**--version-intensity «nombre»**``: Cette option permet de régler l'intensité du scan de version. Une intensité plus basse envoie uniquement les sondes les plus probables, ce qui peut réduire le temps de scan pour les services UDP.

### -O: Détection d'OS

- ``**-O**``: Cette option tente de détecter le système d'exploitation de la cible en analysant les réponses réseau et les caractéristiques des protocoles.
- ``**--osscan-limit**``: Cette option évite de tenter de prédire l'OS si moins de 2 ports (un ouvert, un fermé) sont détectés, ce qui économise du temps de scan.
- ``**--osscan-guess**``: En cas d'incertitude, cette option améliore la détection de l'OS en effectuant des analyses supplémentaires.
